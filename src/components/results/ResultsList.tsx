...
function applyFiltersAndSort(
  ...
  if (min != null) list = list.filter((r) => r.price >= min);

  const max = filters?.priceMax;
  if (max != null) list = list.filter((r) => r.price <= max);
...
export function ResultsList() {
  ...
  const handleSelect = (flight: FlightResult) => {
    // TODO: later: setSelectedFlight(flight) or open details modal, etc.
    alert(`Selected: ${flight.airline} (${flight.airlineCode}) - $${Math.round(flight.price)}`);
  };
    ...
    <div className="space-y-3">
      {results.map((r) => (
        <Card key={r.id} className="p-4">
          ...
          {/* Bottom row: Departure | Arrival | Select (aligned) */}
